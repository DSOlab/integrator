...
/* Case A: Simple STM, no dynamic params (e.g. Cd, Cr, etc)
 * ---------------------------------------------------------
 * we are solving for the EoM + STM combined system of 1st order ODEs, hence at
 * input, y0 should hold:
 * y0.segment<6>(0) aka first six rows should be position and velocity (r, v)
 * The STM matrix is 6x6; hence, we are going to append each (6x1) column of
 * the STM matrix in the y0 column vector. E.g. the first column of the STM
 * will be placed at the [6,12) rows of the y0 column vector, the second column
 * of the STM matrix will be placed at the [12,18) rows of the y0 column vector
 * etc.
 *
 */
